 ...
 * 
 * 按以下类型逐次进行:
 * 1.老风格: 主版本.小版本.revision.构建信息
 * 2.新语义: 主版本.小版本.补丁[-预发布][+构建信息]
 * 3.亦或: 字典序比较
 * 	
 */
int compare_versions(const char* left_version, const char* right_version)
{
	...
	if (!semver_parse(left_version, &left_sem) && !semver_parse(right_version, &right_sem)) {
		DEBUG("Comparing semantic versions '%s' <-> '%s'\n", left_version, right_version);
...
int main() {
	...
	int r;
	r = compare_versions("1.0.0", "1.0.1");
	DEBUG("\n%d\n", r);
	r = compare_versions("1.0.1.234", "1.0.1.123");
	DEBUG("\n%d\n", r);

	r = compare_versions("2024.04-rc4", "2024.04-rc5");
	DEBUG("\n%d\n", r);
	r = compare_versions("2024.05-rc4", "2024.04-rc5");
	DEBUG("\n%d\n", r);
	r = compare_versions("4.4", "4.5");
	DEBUG("\n%d\n", r);
